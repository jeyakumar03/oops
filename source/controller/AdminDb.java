...
public void viewTicket(int busId) throws SQLException {    
        try (PreparedStatement pst = DatabaseConnection.getConnection().prepareStatement(QueriesContainer.Queries.VIEW_TICKET.getQuery())) {
            ...
            ResultSet rs = pst.executeQuery();
            if (!rs.next()) {
                System.out.println("No tickets found for the bus with ID: " + busId);
                return;
            }
            System.out.println("--------------------------------------------------------");
            System.out.println("Ticket Details for Bus ID: " + busId);
            System.out.println("--------------------------------------------------------");
            do {
